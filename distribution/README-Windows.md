...
- Windows 10 or Windows 11 (64-bit) and Google Chrome 116+ or Microsoft Edge
...
1. Keep this extracted folder in a permanent location; Chrome or Edge loads the extension from it.
2. Double-click `Install-Windows.cmd`.
3. Follow the prompt: open `chrome://extensions` (or `edge://extensions`), turn on **Developer mode**, choose **Load unpacked**, and select the included `extension` folder.
...
1. 将解压后的目录保存在固定位置，Chrome 或 Edge 会从这里加载扩展。
2. 双击 `Install-Windows.cmd`。
3. 按提示打开 `chrome://extensions`（或 `edge://extensions`），启用“开发者模式”，点击“加载已解压的扩展程序”，选择包内的 `extension` 文件夹。
...
- Chrome discovers it through `HKCU\Software\Google\Chrome\NativeMessagingHosts\com.codex.gpt_explainer`, and Microsoft Edge through `HKCU\Software\Microsoft\Edge\NativeMessagingHosts\com.codex.gpt_explainer`; the installer registers both by default (use `-Browsers chrome` or `-Browsers edge` to pick one).
- A small per-user launcher forwards the browser's standard input/output to the shared Node.js host. No administrator permission is required.
- When Codex comes from the Microsoft Store, the installer creates a per-user executable copy because browser-launched Node processes cannot execute directly from the protected `WindowsApps` directory.
...
Double-click `Uninstall-Windows.cmd`, then remove the extension from `chrome://extensions` or `edge://extensions`.

双击 `Uninstall-Windows.cmd`，然后在 `chrome://extensions` 或 `edge://extensions` 中移除扩展。
