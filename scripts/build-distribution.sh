...
cp "$PROJECT_ROOT/native-host/install-macos.sh" "$TARGET/native-host/install-macos.sh"
cp "$PROJECT_ROOT/native-host/uninstall-macos.sh" "$TARGET/native-host/uninstall-macos.sh"
cp "$PROJECT_ROOT/distribution/Install.command" "$TARGET/Install.command"
cp "$PROJECT_ROOT/distribution/Uninstall.command" "$TARGET/Uninstall.command"
cp "$PROJECT_ROOT/distribution/README.md" "$TARGET/README.md"
printf '%s\n' "$VERSION" > "$TARGET/VERSION"

chmod 755 "$TARGET/Install.command" "$TARGET/Uninstall.command"
chmod 755 "$TARGET/native-host/install-macos.sh" "$TARGET/native-host/uninstall-macos.sh"
